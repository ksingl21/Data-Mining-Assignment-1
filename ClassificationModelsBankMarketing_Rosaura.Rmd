...
#Obtain predictions
nbPred1 = predict(nbM1, mdTrn, type='prob')

#attempting various thresholds
THRESH1=0.2
table(pred=nbPred1[, 2] > THRESH1, actual=mdTrn$y)
THRESH2=0.5
table(pred=nbPred1[, 2] > THRESH2, actual=mdTrn$y)
THRESH3=0.8
table(pred=nbPred1[, 2] > THRESH3, actual=mdTrn$y)

#ROC curve for nbM1
pred_nbM1=prediction(nbPred1[,2], mdTrn$y)
rocPerf_nbM1 <- performance(pred_nbM1, "tpr","fpr")
plot(rocPerf_nbM1)
abline(a=0, b= 1)

#Lift Curve for nbM1
trnSc_nbM1 <- mdTrn %>%  select("y")   # selects the OUTCOME column into trnSc
trnSc_nbM1$score<-nbPred1[, 2]
trnSc_nbM1<-trnSc_nbM1[order(trnSc_nbM1$score, decreasing=TRUE),]
trnSc_nbM1$cumResponse<-cumsum(trnSc_nbM1$y == "yes")

plot( trnSc_nbM1$cumResponse, type = "l", xlab='#cases', ylab='#default')
abline(0,max(trnSc_nbM1$cumResponse)/nrow(trnSc_nbM1), col="blue")  #diagonal line
